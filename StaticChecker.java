...
public class StaticChecker implements DeclVisitor, StatementVisitor,
    ...
    public void visitAssignmentNode(StatementNode.AssignmentNode node) {
        ...
        for (int i = 0; i < node.getlValues().size(); i++) {
            ...
            ExpNode left = node.getVariable(i).transform(this);
            if (!duplicateChecker.add(left.toString())) {
                staticError(left +" assigned more than once",
                        left.getLocation());
            }
            node.setVariable(i, left);
            ...
            if (left.getType() instanceof Type.ReferenceType) {
                ...
                node.setExp(i, baseType.coerceExp(exp));
    ...
    public void visitCaseNode(CaseNode node) {
        ...
        node.setCond(cond);
        List<StatementNode> cases = node.getCases();
        HashSet<String> duplicateChecker = new HashSet<>();
        for (StatementNode theCase : cases) {
            ((StatementNode.CaseBranchNode) theCase).accept(this, cond,
                    duplicateChecker);
        }
        if(node.getDef() != null) {
     ...
     */
    @Override
    public void visitCaseBranchNode(CaseBranchNode node) {
        beginCheck("CaseBranch");
        ConstExp Case = node.getTheCase();
        ExpNode cond = node.getCond();
        Type baseType = cond.getType();
        HashSet<String> duplicateChecker = node.getDuplicateChecker();
        if (cond.getType() instanceof Type.ReferenceType) {
            baseType = ((Type.ReferenceType) cond.getType()).getBaseType();
        }
        Type check = baseType;
        int lower = 0;
        int upper = 0;
        if (baseType instanceof Type.SubrangeType) {
            Type.SubrangeType typeRange = (Type.SubrangeType) baseType;
            lower = typeRange.getLower();
            upper = typeRange.getUpper();
            currentScope.addConstant("cond", node.getCond().getLocation(),
                    Predefined.INTEGER_TYPE, 0);
            check = Predefined.INTEGER_TYPE;
        }

        if (node.getTheCase().getType().getScalarType() != check) {
            staticError("case label type does not match case " +
                    "expression type", Case.loc);
        } else {
            if (duplicateChecker.add(String.valueOf(Case.value))) {
                if (baseType instanceof Type.SubrangeType &&(
                Case.getValue() > upper ||
                Case.getValue() < lower)) {

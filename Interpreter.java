...
public class Interpreter implements StatementVisitor, ExpTransform<Value> {
    ...
    public void visitAssignmentNode(StatementNode.AssignmentNode node) {
        ...
        for (ExpNode Exp: node.getExps()) {
            Value value = Exp.evaluate(this);
            values.add(value);
        }
        for(int i = 0; i < node.getExps().size(); i++) {
            /* Evaluate the code to be assigned */
            /* Assign the value to the variables offset */
            Value lValue = node.getVariable(i).evaluate(this);
            assignValue(lValue, values.get(i));
        }
        endExec("Assignment");
    }
    ...
    /**
     * Execute code for skip statement
     */
    @Override
    ...
    /**
     * Execute code for case statement
     */
    @Override
    public void visitCaseNode(StatementNode.CaseNode node) {
        beginExec("Case");
        Value value = node.getCond().evaluate(this);
        node.setClause(null);
        if (value.toString().contains("address")) {
            value = currentFrame.lookupFrame(value.getAddressLevel())
                        .lookup(value.getAddressOffset());
        }
        List<StatementNode> cases = node.getCases();
        for (int i = 0; i < cases.size(); i++) {
            StatementNode.CaseBranchNode theCase =
                    (StatementNode.CaseBranchNode) cases.get(i);
            int caseValue = theCase.getTheCase().getValue();
            if (value.getInteger() == caseValue) {
                theCase.getClause().accept(this);
                node.setClause(theCase.getClause());
            }
        }
        if (node.getClause() == null && node.getDef() != null) {
            node.getDef().accept(this);
        }
        if (node.getClause() == null && node.getDef() == null) {
            runtime("expression in case doesn't match any label",
                    node.getCond().getLocation(), currentFrame);
        }
        endExec("Case");
    }

    /**
     * Execute code for case branch
     */
    @Override
    public void visitCaseBranchNode(StatementNode.CaseBranchNode node) {
        beginExec("CaseBranch");

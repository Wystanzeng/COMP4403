...
public class Parser {
    ...
    private ExpNode parseExp(TokenSet recoverSet) {
        return exp.parse("Expression", EXP_START_SET, recoverSet,
                () -> {
                    ...
                    if (haveUnaryMinus) {
                        exp = new ExpNode
                                .UnaryNode(loc, Operator.NEG_OP, exp);
                    }
    ...
    private final ParseMethod<StatementNode.AssignmentNode> assign =
            new ParseMethod<>(
                    (Location loc) -> {
                        List<ExpNode> left = new ArrayList<>();
                        left.add(new ExpNode.ErrorNode(loc));
                        List<ExpNode> right = new ArrayList<>();
    ...
    /**
     * Rule: parseCaseStatement -> KW_CASE condition OF
     */
    private StatementNode parseCaseStatement(TokenSet recoverSet) {
        return stmt.parse("Case Statement", Token.KW_CASE, recoverSet,
                () -> {
                    ...
                    Location loc = tokens.getLocation();
                    ExpNode cond =
                            parseCondition(recoverSet.union(Token.KW_OF));
                    List<StatementNode> cases = new ArrayList<>();
                    tokens.match(Token.KW_OF);
                    while (tokens.isMatch(Token.KW_WHEN)) {
                        cases.add(parseCaseBranch(recoverSet));
                    }
                    StatementNode def = null;
                    if (tokens.isMatch(Token.KW_DEFAULT)) {
                        tokens.match(Token.KW_DEFAULT, STATEMENT_START_SET);
                        def = parseStatementList(recoverSet);
                    }
                    if (tokens.isMatch(Token.KW_END)) {
                        tokens.match(Token.KW_END);
                    }
                    return new StatementNode.CaseNode(loc, cond, cases, def);
                });
    }

    /**
     * Rule: parseCaseBranch -> KW_WHEN constant COLON statement
     */
    private StatementNode parseCaseBranch(TokenSet recoverSet) {
        return  stmt.parse("Case Branch", Token.KW_WHEN, recoverSet,
                () -> {
                    ...
                    Location loc = tokens.getLocation();
                    ConstExp theCase =
                            parseConstant(recoverSet.union(Token.COLON));
                    tokens.match(Token.COLON, STATEMENT_START_SET);
                    StatementNode clause = parseStatementList(recoverSet);
                    return new StatementNode.CaseBranchNode(loc, theCase, clause);
                });
    }
